...
- (id <SDWebImageOperation>)downloadImageWithURL:(NSURL *)url
    ...
    operation.cacheOperation = [self.imageCache queryDiskCacheForKey:key clipStrategy:url.clipStrategy  done:^(UIImage *image, SDImageCacheType cacheType) {
            ...
            id <SDWebImageOperation> subOperation = [self.imageDownloader downloadImageWithURL:url options:downloaderOptions progress:progressBlock completed:^(UIImage *downloadedImage, NSData *data, NSError *error, BOOL finished) {
                if (weakOperation.isCancelled) {
                    ...
                    if (options & SDWebImageRefreshCached && image && !downloadedImage) {
                        ...
                        UIImage* retImg = downloadedImage;
                        
                        if (retImg && finished) {
                            retImg = [self.imageCache storeImage:downloadedImage clipStrategy:url.clipStrategy recalculateFromImage:NO imageData:data forKey:key toDisk:cacheOnDisk];
                        }

                        
                        dispatch_main_sync_safe(^{
                            if (!weakOperation.isCancelled) {
                                completedBlock(retImg, nil, SDImageCacheTypeNone, finished, url);
                            }
                        });
